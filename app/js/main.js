...
var lang_locale = navigator.language.split('-')[0],
    ...
    dialogCallCount = 0,
    breakpointsChat = [2, 3, 4, 5, 8, 19],
    //для того чтобы не останавливаться на одном отправленном сообщение от доктора, если нужно отправить несклько
Chat = {
    ru: {
        typing: 'печатает',
...
var v_chat = new Vue({
    ...
    methods: {
        getTypeMes: function getTypeMes(type) {
            if (type === 'user') return 'my-message';
        },
        setActiveDoctor: function setActiveDoctor(number) {
            if (number === 2) {
    ...
    sendMesFromDoctor: function sendMesFromDoctor() {
        ...
        if (dialogCallCount >= 10 && dialogCallCount <= 15) {
            //если сообщение перепутье между true и false, то определяем результат имт
    ...
    sendMesFromUser: function sendMesFromUser(message) {
        var mesUser = {
            type: 'user',
            text: message,
            date: nowDate.getFullTime()
    ...
    validation: function validation(event, textarea) {
        var breakpointsChatValidation = [3, 4, 5, 8],
            //ограничить ввод только цифрами [ age, weight, height ]
        ...
        if (currentMessageNumber != -1) {
            if (event.keyCode < 48 || event.keyCode > 57) {
                ...
                var checkSymbols = function checkSymbols(maxvalue, element, errorMessage) {
    ...
    analysisIMT: function analysisIMT(imt) {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    [].forEach.call($chooseDoctor, function (current) {
        current.addEventListener('click', function () {
            ...
            var doctorId = +current.getAttribute('data-doctor');
            v_chat.setActiveDoctor(doctorId);

            if (doctorId === 1) {
                //если активный первый врач
